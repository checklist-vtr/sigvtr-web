...
function draftSnapshot(){const fields={};draftFields().forEach(id=>{const el=$("#"+id);if(!el)return;fields[id]=el.type==="checkbox"?!!el.checked:el.value});const multiAlterations={};Object.keys(state.multiAlterations||{}).forEach(key=>{multiAlterations[key]=(state.multiAlterations[key]||[]).map(item=>({id:String(item.id),description:String(item.description||"")}))});return {version:2,savedAt:Date.now(),step:state.step,requestId:state.requestId||"",fields,status:{...state.status},descriptions:{...state.descriptions},multiAlterations,decisions:{...state.decisions}}}
...
async function restoreDraft(){let draft;try{const raw=localStorage.getItem(DRAFT_KEY);if(!raw)return false;draft=JSON.parse(raw)}catch(_){return false}if(!draft||![1,2].includes(Number(draft.version))||!draft.fields||typeof draft.fields!=="object")return false;draftRestoring=true;try{for(const [id,value] of Object.entries(draft.fields)){const el=$("#"+id);if(!el)continue;if(el.type==="checkbox")el.checked=!!value;else el.value=String(value??"")}const otherPrefix=$("#otherPrefixWrap"),otherOperation=$("#otherOperationWrap");if(otherPrefix)otherPrefix.hidden=$("#prefixoSelect").value!=="OUTRO";if(otherOperation)otherOperation.hidden=$("#turno").value!=="OUTROS";state.status={};state.descriptions={};state.multiAlterations={};for(const [key,status] of Object.entries(draft.status||{})){const card=document.querySelector(`.inspection-card[data-key="${CSS.escape(key)}"]`);if(card&&["ok","nao","na"].includes(status))setStatus(card,status)}for(const [key,value] of Object.entries(draft.descriptions||{})){if(isMultiOtherKey(key))continue;const card=document.querySelector(`.inspection-card[data-key="${CSS.escape(key)}"]`),ta=card&&card.querySelector("textarea");if(card&&ta&&state.status[key]==="nao"){const clean=sanitizeDescription(String(value||""));ta.value=clean;state.descriptions[key]=clean.trim()}}for(const key of MULTI_OTHER_KEYS){if(state.status[key]!=="nao")continue;const saved=Array.isArray(draft.multiAlterations&&draft.multiAlterations[key])?draft.multiAlterations[key]:[];const legacy=String((draft.descriptions||{})[key]||"").trim();state.multiAlterations[key]=(saved.length?saved:(legacy?[{id:"1",description:legacy}]:[{id:"1",description:""}])).map((item,index)=>({id:/^\d{1,6}$/.test(String(item&&item.id||""))?String(item.id):String(index+1),description:sanitizeDescription(String(item&&item.description||"")).slice(0,300)}));const card=document.querySelector(`.inspection-card[data-key="${CSS.escape(key)}"]`);if(card)renderMultiAlterations(card)}state.requestId=String(draft.requestId||"");if(getPrefix())await loadPending();state.decisions={};for(const [id,decision] of Object.entries(draft.decisions||{}))if(state.pending.some(d=>String(d.idAvaria)===String(id))&&["continua","agravou","solicitar_verificacao"].includes(decision))state.decisions[id]=decision;renderKnownDamages();const step=Number(draft.step);showStep(Number.isInteger(step)&&step>=1&&step<=STEPS.length?step:1);toast("Checklist em andamento recuperado. Reinsira as fotos necessárias.");return true}catch(_){return false}finally{draftRestoring=false}}
...
function renderGroup(id,items){$("#"+id).innerHTML=items.map(item=>{const multi=isMultiOtherKey(item.key);const panel=multi?`<div class="change-panel multi-change-panel" hidden><div class="multi-alterations"></div><button type="button" class="add-other-alteration" data-add-other="${item.key}">+ ADICIONAR OUTRA ALTERAÇÃO</button></div>`:`<div class="change-panel" hidden><textarea maxlength="300" placeholder="Descreva a alteração encontrada"></textarea><div class="inline-photo"><button type="button" data-capture="avaria_${item.key}">INSERIR FOTO DA ALTERAÇÃO</button><input id="photo_avaria_${item.key}" type="file" accept="image/*" capture="environment" hidden><div class="photo-preview"></div></div></div>`;return `<article class="inspection-card" data-key="${item.key}"><h3>${escapeHtml(item.name)}</h3><p class="item-note">${escapeHtml(item.note|| (item.optional?"Opcional — use Não se aplica quando ausente de fábrica.":"Selecione a condição encontrada."))}</p><div class="known-damages"></div><div class="additional-damage-question" hidden></div><div class="status-buttons"><button type="button" class="status-choice ok" data-status="ok">SEM ALTERAÇÃO</button><button type="button" class="status-choice change" data-status="nao">COM ALTERAÇÃO</button>${item.optional?'<button type="button" class="status-choice na" data-status="na">NÃO SE APLICA</button>':""}</div>${panel}</article>`}).join("")}
function isMultiOtherKey(key){return MULTI_OTHER_KEYS.has(String(key||""))}
function ensureMultiAlterations(key){if(!Array.isArray(state.multiAlterations[key])||!state.multiAlterations[key].length)state.multiAlterations[key]=[{id:"1",description:""}];return state.multiAlterations[key]}
function nextMultiAlterationId(key){const used=ensureMultiAlterations(key).map(item=>Number(item.id)||0);return String(Math.max(0,...used)+1)}
function multiPhotoType(key,id){return `avaria_${key}_${id}`}
function renderMultiAlterations(card){if(!card)return;const key=card.dataset.key,host=card.querySelector(".multi-alterations");if(!host)return;const list=ensureMultiAlterations(key);host.innerHTML=list.map((item,index)=>{const type=multiPhotoType(key,item.id);return `<section class="multi-alteration" data-multi-id="${escapeHtml(item.id)}"><div class="multi-alteration-head"><strong>Alteração ${index+1}</strong>${list.length>1?`<button type="button" class="remove-other-alteration" data-remove-other="${key}" data-remove-other-id="${escapeHtml(item.id)}">REMOVER</button>`:""}</div><textarea maxlength="300" data-multi-description="${escapeHtml(item.id)}" placeholder="Descreva a alteração encontrada">${escapeHtml(item.description||"")}</textarea><div class="inline-photo"><button type="button" data-capture="${type}">INSERIR FOTO DA ALTERAÇÃO</button><input id="photo_${type}" type="file" accept="image/*" capture="environment" hidden><div class="photo-preview"></div></div></section>`}).join("");list.forEach(item=>renderPhoto(multiPhotoType(key,item.id)))}
function addMultiAlteration(key){const card=document.querySelector(`.inspection-card[data-key="${CSS.escape(key)}"]`);if(!card||state.status[key]!=="nao")return;ensureMultiAlterations(key).push({id:nextMultiAlterationId(key),description:""});renderMultiAlterations(card);scheduleDraftSave()}
function removeMultiAlteration(key,id){const list=ensureMultiAlterations(key);if(list.length<=1)return;const index=list.findIndex(item=>String(item.id)===String(id));if(index<0)return;delete state.photos[multiPhotoType(key,list[index].id)];list.splice(index,1);const card=document.querySelector(`.inspection-card[data-key="${CSS.escape(key)}"]`);if(card)renderMultiAlterations(card);scheduleDraftSave()}
function multipleAlterationsPayload(){const result={};for(const key of MULTI_OTHER_KEYS){if(state.status[key]!=="nao")continue;result[key]=ensureMultiAlterations(key).map(item=>({id:String(item.id),descricao:String(item.description||"").trim(),fotoTipo:multiPhotoType(key,item.id)}))}return result}
function descriptionsPayload(){const result={...state.descriptions};for(const key of MULTI_OTHER_KEYS){if(state.status[key]!=="nao")continue;const first=ensureMultiAlterations(key)[0];result[key]=String(first&&first.description||"").trim()}return result}
...
function bind(){
  document.addEventListener("click",e=>{
    ...
    const n=e.target.closest(".next-button"),p=e.target.closest(".prev-button"),c=e.target.closest(".status-choice"),cap=e.target.closest("[data-capture]"),rm=e.target.closest(".remove-photo"),dd=e.target.closest("[data-damage-decision]"),addOther=e.target.closest("[data-add-other]"),removeOther=e.target.closest("[data-remove-other-id]");
    ...
    if(addOther){addMultiAlteration(addOther.dataset.addOther)}
...
function setStatus(card,status){const key=card.dataset.key;state.status[key]=status;card.classList.toggle("is-ok",status==="ok"||status==="na");card.classList.toggle("is-change",status==="nao");card.querySelectorAll(".status-choice").forEach(b=>b.classList.toggle("selected",b.dataset.status===status));card.querySelector(".change-panel").hidden=status!=="nao";if(isMultiOtherKey(key)){if(status==="nao"){ensureMultiAlterations(key);renderMultiAlterations(card)}else{delete state.multiAlterations[key];Object.keys(state.photos).filter(type=>type.startsWith(`avaria_${key}_`)).forEach(type=>delete state.photos[type])}return}if(status!=="nao"){delete state.descriptions[key];delete state.photos["avaria_"+key];const ta=card.querySelector("textarea");if(ta)ta.value="";renderPhoto("avaria_"+key)}}
...
function compressImage(file){return new Promise((resolve,reject)=>{
 ...
 img.onerror=()=>{URL.revokeObjectURL(url);reject(new Error("Imagem inválida ou não suportada."))};img.src=url
})}
...
function validateStep(step){hideErrors();if(step===1){const prefix=getPrefix(),name=$("#condutor").value.trim(),rank=$("#postoGraduacao").value,rg=$("#rg").value.trim(),km=$("#kmInicial").value.trim(),turno=$("#turno").value,otherOperation=$("#otherOperation").value.trim(),fuel=$("#combustivel").value;let msg="";if(!prefix||(!FIXED_PREFIX.test(prefix)&&!EXTERNAL_PREFIX.test(prefix)))msg="Selecione uma viatura ou informe um prefixo externo somente numérico.";else if(!NAME_PATTERN.test(name))msg="O nome de guerra deve conter somente letras e espaços.";else if(!RANK_LABELS[rank])msg="Selecione o Posto/Graduação.";else if(!/^\d{1,20}$/.test(rg))msg="O RG deve conter somente números.";else if(!/^\d{1,9}$/.test(km))msg="O KM deve conter somente números.";else if(!turno)msg="Selecione o turno.";else if(turno==="OUTROS"&&!DESC_PATTERN.test(otherOperation))msg="Informe corretamente a operação ou o turno em Outros.";else if(!fuel)msg="Selecione o nível de combustível.";else if(!state.photos.painel_inicial)msg="Insira uma foto legível mostrando, na mesma imagem, o odômetro e o marcador de combustível.";if(msg)return error("identificationError",msg),false;return true}if(step>=2&&step<=4){const items=step===2?ITEMS.external:step===3?ITEMS.internal:ITEMS.engine;for(const item of items){const st=state.status[item.key];if(!st)return sectionError(step,`Avalie o item: ${item.name}`),false;if(st==="nao"){if(isMultiOtherKey(item.key)){const entries=ensureMultiAlterations(item.key);for(let i=0;i<entries.length;i++){const entry=entries[i];if(!DESC_PATTERN.test(String(entry.description||"")))return sectionError(step,`Descreva corretamente a alteração ${i+1} em: ${item.name}`),false;if(!state.photos[multiPhotoType(item.key,entry.id)])return sectionError(step,`Insira a foto da alteração ${i+1} em: ${item.name}`),false}}else{if(!DESC_PATTERN.test(state.descriptions[item.key]||""))return sectionError(step,`Descreva corretamente a alteração em: ${item.name}`),false;if(!state.photos["avaria_"+item.key])return sectionError(step,`Insira a foto da alteração em: ${item.name}`),false}}}const related=state.pending.filter(d=>items.some(i=>i.key===matchDamageToKey(d)));for(const d of related)if(!state.decisions[d.idAvaria])return sectionError(step,"Informe a situação das avarias já registradas exibidas nesta etapa."),false;return true}if(step===5){for(const p of FINAL_PHOTOS)if(!state.photos[p.type])return error("finalPhotosError",`Insira a foto obrigatória: ${p.label}.`),false;return true}return true}
...
function renderSummary(){
 ...
 $("#summaryContent").innerHTML=`<div class="summary-block"><h3>Identificação</h3><p><strong>Viatura:</strong> ${escapeHtml(getPrefix())}</p><p><strong>Condutor:</strong> ${escapeHtml(driver)}</p><p><strong>RG:</strong> ${escapeHtml($("#rg").value)}</p><p><strong>KM:</strong> ${escapeHtml($("#kmInicial").value)}</p><p><strong>Turno:</strong> ${escapeHtml(SHIFT_LABELS[$("#turno").value]||$("#turno").value)}${$("#turno").value==="OUTROS"?" — "+escapeHtml($("#otherOperation").value):""}</p><p><strong>Combustível:</strong> ${escapeHtml($("#combustivel").value)}</p></div><div class="summary-block"><h3>Novas alterações registradas</h3><p><strong>Itens avaliados:</strong> ${Object.keys(state.status).length}</p><p><strong>Com alteração:</strong> ${changed.length}</p>${changed.map(k=>isMultiOtherKey(k)?ensureMultiAlterations(k).map((entry,index)=>`<p><strong>${escapeHtml(itemName(k))} — Alteração ${index+1}:</strong> ${escapeHtml(entry.description||"")}</p>`).join(""):`<p><strong>${escapeHtml(itemName(k))}:</strong> ${escapeHtml(state.descriptions[k])}</p>`).join("")||"<p>Sem novas alterações.</p>"}${newDamageHtml}</div><div class="summary-block"><h3>Avarias pendentes conhecidas</h3>${knownHtml}</div>`;
}
...
function buildSubmissionPayload(){
 ...
 return {action:"salvarRetiradaMobile",data:{
  ...
  itens:{...state.status},
  descricoesAlteracoes:descriptionsPayload(),
  alteracoesMultiplas:multipleAlterationsPayload(),
  avariasConhecidas,
  fotos,
...
function registerSW(){if("serviceWorker" in navigator)navigator.serviceWorker.register("sw.js?v=1.18.5-rc1").catch(()=>{})}
