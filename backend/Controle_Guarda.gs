...
function guardShiftLocatorById_(turnoId) {
  ...
  const sh=getSpreadsheet_().getSheetByName(SIGVTR.SHEETS.GUARD_SHIFTS),hm=guardHeaderMap_(sh);if(hm.map.ID_TURNO===undefined||sh.getLastRow()<2)return null;
  const found=sh.getRange(2,hm.map.ID_TURNO+1,sh.getLastRow()-1,1).createTextFinder(id).matchEntireCell(true).findNext();if(!found)return null;
  const rowIndex=found.getRow(),row=sh.getRange(rowIndex,1,1,hm.headers.length).getValues()[0];
  return {sheet:sh,map:hm.map,rowIndex:rowIndex,row:row,turno:guardShiftFromRow_(row,hm.map)};
}
...
function openGuardShift_(operator,options) {
  ...
  if(existing&&options.forceNew){
    const loc=guardShiftLocatorById_(existing.id);loc.sheet.getRange(loc.rowIndex,loc.map.STATUS+1).setValue(GUARDA.STATUS_TURNO.PENDENTE);guardInvalidateContextCache_();
  }
...
function guardContextSnapshot_(forceFresh){
  if(!forceFresh){const cached=guardCacheGetJson_(GUARDA_CACHE.CONTEXT);if(cached&&Array.isArray(cached.turnosPendentes)&&Array.isArray(cached.movimentacoes))return {turno:cached.turno||null,turnosPendentes:cached.turnosPendentes,movimentacoes:cached.movimentacoes,_movRows:null,_movMap:null,cached:true};}
  ...
  const compact={turno:open,turnosPendentes:pending,movimentacoes:movements};guardCachePutJson_(GUARDA_CACHE.CONTEXT,compact,GUARDA_CACHE.CONTEXT_SECONDS);
  return {turno:open,turnosPendentes:pending,movimentacoes:movements,_movRows:movRows,_movMap:movHm.map,cached:false};
}
...
function guardMovementLocator_(id) {
  const sh=getSpreadsheet_().getSheetByName(SIGVTR.SHEETS.GUARD_MOVEMENTS),hm=guardHeaderMap_(sh),wanted=guardText_(id,100);if(!wanted||hm.map.ID_MOVIMENTACAO===undefined||sh.getLastRow()<2)return null;
  const found=sh.getRange(2,hm.map.ID_MOVIMENTACAO+1,sh.getLastRow()-1,1).createTextFinder(wanted).matchEntireCell(true).findNext();if(!found)return null;
  const rowIndex=found.getRow(),row=sh.getRange(rowIndex,1,1,hm.headers.length).getValues()[0];
  return {sheet:sh,map:hm.map,headers:hm.headers,rowIndex:rowIndex,row:row,movimento:guardMovementFromRow_(row,hm.map)};
}
...
function guardCloseShiftWrite_(loc,data,substituto) {
  ...
  const range=loc.sheet.getRange(loc.rowIndex,1,1,row.length);range.setValues([row]);guardInvalidateContextCache_();
...
function generateGuardShiftPdf_(turnoId,opts){
  ...
  doc.saveAndClose();
  const docFile=DriveApp.getFileById(doc.getId()),blob=docFile.getAs(MimeType.PDF),stamp=Utilities.formatDate(new Date(turno.fimEm||turno.inicioEm||new Date()),SIGVTR.TIMEZONE,'dd-MM-yyyy_HH-mm'),filename=sanitizeFilename_('controle-da-guarda_'+stamp+'.pdf');blob.setName(filename);
...
/******************************************************************
 * Controle da Guarda - integração com Admin > Relatórios
 * v0.7.0
 ******************************************************************/
function buildReportGuardV2_(p){
  p=p||{};ensureGuardStructure_();
  const sh=getSpreadsheet_().getSheetByName(SIGVTR.SHEETS.GUARD_SHIFTS);
  const hm=guardHeaderMap_(sh),rows=sh.getDataRange().getValues();
  const start=String(p.dataInicial||'').trim()?new Date(String(p.dataInicial)+'T00:00:00'):null;
  const end=String(p.dataFinal||'').trim()?new Date(String(p.dataFinal)+'T23:59:59.999'):null;
  const defs=[['dataServico','Data'],['inicio','Início'],['termino','Término'],['status','Situação'],['responsavel','Responsável pelo encerramento'],['movimentacoes','Movimentações'],['devolvidas','Devolvidas'],['emUso','Em uso no encerramento'],['recebidasTurnoAnterior','Recebidas de turno anterior'],['pdf','PDF']];
  const out=[];
  for(let r=1;r<rows.length;r++){
    const turno=guardShiftFromRow_(rows[r],hm.map);
    if(!turno||!turno.id||!turno.inicioEm)continue;
    const dt=new Date(turno.inicioEm);if(isNaN(dt.getTime()))continue;
    if(start&&dt<start)continue;if(end&&dt>end)continue;
    const resumo=getGuardShiftSummary_(turno.id);
    const isSub=turno.status===GUARDA.STATUS_TURNO.FECHADO_SUBSTITUTO;
    const responsavel=isSub?[turno.encerradoPorPostoGraduacao,turno.encerradoPorNome].filter(Boolean).join(' '):[turno.comandantePostoGraduacao,turno.comandanteNome].filter(Boolean).join(' ');
    const statusMap={ABERTO:'ABERTO',PENDENTE_ENCERRAMENTO:'PENDENTE',FECHADO:'FECHADO',FECHADO_POR_SUBSTITUTO:'FECHADO POR OUTRO MILITAR'};
    out.push({
      turnoId:turno.id,
      dataServico:guardPdfDate_(turno.inicioEm,false),
      inicio:guardPdfDate_(turno.inicioEm,true),
      termino:turno.fimEm?guardPdfDate_(turno.fimEm,true):'—',
      status:statusMap[turno.status]||turno.status||'—',
      responsavel:responsavel||'—',
      movimentacoes:Number(resumo.movimentacoes||0),
      devolvidas:Number(resumo.devolvidas||0),
      emUso:Number(resumo.emUso||0),
      recebidasTurnoAnterior:Number(resumo.devolucoesTurnoAnterior||0),
      pdf:turno.pdfFileId?'DISPONÍVEL':((turno.status===GUARDA.STATUS_TURNO.FECHADO||turno.status===GUARDA.STATUS_TURNO.FECHADO_SUBSTITUTO)?'GERAR':'INDISPONÍVEL'),
      pdfGerado:!!turno.pdfFileId,
      fechado:turno.status===GUARDA.STATUS_TURNO.FECHADO||turno.status===GUARDA.STATUS_TURNO.FECHADO_SUBSTITUTO,
      _time:dt.getTime()
    });
  }
  out.sort(function(a,b){return b._time-a._time;});out.forEach(function(x){delete x._time;});
  const summary={registros:out.length,fechados:out.filter(function(x){return x.fechado;}).length,pendentes:out.filter(function(x){return x.status==='PENDENTE';}).length,abertos:out.filter(function(x){return x.status==='ABERTO';}).length};
  return reportFinishV2_('Controle da Guarda',defs,out,p,summary);
}

function testarControleGuardaEtapa7(){
  ensureGuardStructure_();
  const r=buildReportGuardV2_({dataInicial:'',dataFinal:'',colunas:[]});
  return {success:GUARDA.MODULE_VERSION==='0.7.0'&&Array.isArray(r.registros),moduleVersion:GUARDA.MODULE_VERSION,integracaoAdminRelatorios:true,turnosEncontrados:r.registros.length,pdfRegeneravel:true};
}
...
function testarControleGuardaEtapa6(){
  ...
  return {success:cols&&GUARDA.MODULE_VERSION==='0.6.6',moduleVersion:GUARDA.MODULE_VERSION,colunasPdfOk:cols,pastaRelatorios:'SIGVTR - Controle da Guarda / Relatorios',pdfRegeneravel:true};
}
...
function testarControleGuardaCorrecao063() {
  ...
  return {
    ...
    nomePdf:'controle-da-guarda_dd-MM-yyyy_HH-mm.pdf'
  };
}
...
function testarControleGuardaCorrecao065(){
  ...
  return {success:GUARDA.MODULE_VERSION==='0.6.6',moduleVersion:GUARDA.MODULE_VERSION,pdfIncluiVtrHerdadaEmUso:true,resumoAlinhadoAoPainel:true,turnoAbertoId:aberto&&aberto.id||'',movimentacoesVisiveisNoTurno:snap?snap.resumo.movimentacoes:0,emUsoVisiveisNoTurno:snap?snap.resumo.emUso:0};
}

function testarControleGuardaPerformance066(){
