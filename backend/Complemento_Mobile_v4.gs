...
function sanitizeMobileWithdrawalData_(input){
  ...
  const data={};data.tipoChecklist=sanitizeChecklistType_(input.tipoChecklist);data.idRequisicao=sanitizeRequestId_(input.idRequisicao);data.prefixo=sanitizeVehiclePrefix_(input.prefixo);data.dataCliente=sanitizeDateText_(input.dataCliente);
