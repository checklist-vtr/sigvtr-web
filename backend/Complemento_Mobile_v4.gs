...
function saveMobileWithdrawal_(input){
  const data=sanitizeMobileWithdrawalData_(input);validateMobileWithdrawal_(data);
  const ss=getSpreadsheet_(),existing=findMobileWithdrawalByRequestId_(ss,data.idRequisicao);if(existing){const confirmed=confirmMobileWithdrawalByRequestId_(data.idRequisicao);return confirmed&&confirmed.found?confirmed:{success:true,id:existing.id,protocolo:existing.protocolo,status:existing.status,fotosSalvas:existing.fotosSalvas||0,novasAvarias:existing.novasAvarias||0,repetido:true};}
  ...
  appendLog_(ss,{idUsuario:"",action:"CHECKLIST CONDUTOR",referenceId:idWithdrawal,description:"Checklist "+status+" - "+data.prefixo+" - "+protocol,device:data.dispositivo||{},result:"SUCESSO",now:now});
...
function sanitizeMobileWithdrawalData_(input){
  ...
  const data={};data.tipoChecklist="CONDUTOR";data.idRequisicao=sanitizeRequestId_(input.idRequisicao);data.prefixo=sanitizeVehiclePrefix_(input.prefixo);data.dataCliente=sanitizeDateText_(input.dataCliente);
...
function findMobileWithdrawalByRequestId_(ss,requestId){const sheet=requireSheet_(ss,SIGVTR.SHEETS.WITHDRAWALS);if(sheet.getLastRow()<2)return null;const data=sheet.getDataRange().getValues(),headers=data.shift().map(function(h){return String(h).trim();}),jsonIndex=headers.indexOf("ITENS_JSON"),idIndex=headers.indexOf("ID_RETIRADA"),protocolIndex=headers.indexOf("Protocolo"),statusIndex=headers.indexOf("Status");if(jsonIndex<0)return null;for(let i=data.length-1;i>=0;i--){const raw=String(data[i][jsonIndex]||"");if(raw.indexOf(requestId)===-1)continue;try{const parsed=JSON.parse(raw);if(parsed.idRequisicao===requestId)return{id:idIndex>=0?data[i][idIndex]:"",protocolo:protocolIndex>=0?data[i][protocolIndex]:"",status:statusIndex>=0?data[i][statusIndex]:"",fotosSalvas:0,novasAvarias:0};}catch(_){}}return null;}
function confirmMobileWithdrawalByRequestId_(requestId){
  ...
  for(let i=values.length-1;i>=0;i--){
    ...
    try{
      ...
      const alertId=createAdminAlert_({tipo:"CHECKLIST",idReferencia:id,idVtr:idVtr,prefixo:prefix,condutor:optionalCell_(row,idx,"Motorista")||"",postoGraduacao:optionalCell_(row,idx,"Posto/Graduação")||parsed.postoGraduacao||"",rg:optionalCell_(row,idx,"RG PMPA")||"",km:optionalCell_(row,idx,"KM Inicial")||0,titulo:"Novo checklist",descricao:"Checklist do condutor concluído com status "+status+".",now:optionalCell_(row,idx,"Data/Hora Registro")||new Date()});
      return {found:true,success:true,id:id,protocolo:protocol,status:status,fotosSalvas:0,novasAvarias:0,repetido:true,confirmacaoRecuperada:true,idAlerta:alertId||""};
    }catch(_){}
  }
  return {found:false};
}
...
function appendMobileWithdrawal_(ss,c){const sheet=requireSheet_(ss,SIGVTR.SHEETS.WITHDRAWALS);ensureExtraWithdrawalColumns_(sheet);const headers=requireHeaders_(sheet,["ID_RETIRADA","ID_USUARIO","ID_VTR","Tipo_Retirada","KM Inicial","Fotos Obrigatórias","Status","Data/Hora Registro","Motorista","Posto/Graduação","RG PMPA","ITENS_JSON"]),v={};v["ID_RETIRADA"]=c.idWithdrawal;v["ID_USUARIO"]="";v["ID_VTR"]=c.vehicle.id||"";v["Tipo_Retirada"]="CHECKLIST CONDUTOR";v["KM Inicial"]=Number(c.data.kmInicial);v["Fotos Obrigatórias"]="SIM";v["Status"]=c.status;v["Data/Hora Registro"]=c.now;v["Motorista"]=c.data.condutor;v["Posto/Graduação"]=c.data.postoGraduacao;v["RG PMPA"]=c.data.rg;v["Combustível Inicial"]=c.data.combustivel;v["Turno"]=c.data.turno;v["Operação/Outros"]=c.data.operacaoOutro||"";v["Tipo Checklist"]="CONDUTOR";v["Observações"]=buildAlterationSummary_(c.data);v["Protocolo"]=c.protocol;v["Dispositivo"]=(c.data.dispositivo||{}).tipo||"";v["Navegador"]=(c.data.dispositivo||{}).navegador||"";v["ITENS_JSON"]=JSON.stringify({idRequisicao:c.data.idRequisicao,tipoChecklist:"CONDUTOR",itens:c.data.itens,descricoes:c.data.descricoesAlteracoes,combustivel:c.data.combustivel,turno:c.data.turno,operacaoOutro:c.data.operacaoOutro,postoGraduacao:c.data.postoGraduacao,dataCliente:c.data.dataCliente,prefixo:c.data.prefixo});appendByHeaders_(sheet,headers,v);}
