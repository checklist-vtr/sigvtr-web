...
function checkPreventiveReviewForVehicle_(idVtr,prefixo,kmAtual,now){
  ensureAdminSheets_();
  const ss=getSpreadsheet_(),sh=requireSheet_(ss,"REVISOES"),data=sh.getDataRange().getValues(),heads=data.shift().map(String),ii=heads.indexOf("ID_VTR"),pi=heads.indexOf("Prefixo"),li=heads.indexOf("KM Última Revisão"),invi=heads.indexOf("Intervalo KM"),ni=heads.indexOf("Próxima Revisão KM"),ai=heads.indexOf("Antecedência Alerta KM"),si=heads.indexOf("Status");
  let rowIndex=-1,row=null;
  for(let i=0;i<data.length;i++){
    if(String(data[i][ii])===String(idVtr)||String(data[i][pi]).toUpperCase()===String(prefixo).toUpperCase()){rowIndex=i+2;row=data[i];break;}
  }
  if(!row){
    const headers=requireHeaders_(sh,ADMIN_REVIEW_HEADERS),interval=10000,next=Math.max(Number(kmAtual)||0,0)+interval,lead=1000;
    appendByHeaders_(sh,headers,{"ID_REVISAO":"REV-"+Utilities.getUuid(),"ID_VTR":idVtr,"Prefixo":prefixo,"KM Última Revisão":Number(kmAtual)||0,"Intervalo KM":interval,"Próxima Revisão KM":next,"Antecedência Alerta KM":lead,"Data Última Revisão":"","Status":"ATIVA","Observação":"Cadastro automático. Ajuste a próxima revisão no cadastro da viatura.","Última Atualização":now});
    return;
  }
  const interval=Number(row[invi])||10000,last=Number(row[li])||0,next=Number(row[ni])||last+interval,lead=Math.max(0,Number(ai>=0?row[ai]:1000)||1000),status=String(si>=0?row[si]:"ATIVA").toUpperCase();
  if(!next||status==="ARQUIVADA"||status==="INATIVA")return;
  const remaining=next-kmAtual;
  if(kmAtual>=next){
    createAdminAlert_({tipo:"REVISAO",idReferencia:"REVISAO_VENCIDA:"+idVtr+":"+next,idVtr:idVtr,prefixo:prefixo,km:kmAtual,proximaRevisao:next,titulo:"Revisão preventiva vencida",descricao:"A viatura atingiu ou ultrapassou a quilometragem prevista para revisão.",now:now});
  }else if(remaining<=lead){
    createAdminAlert_({tipo:"REVISAO",idReferencia:"REVISAO_PROXIMA:"+idVtr+":"+next,idVtr:idVtr,prefixo:prefixo,km:kmAtual,proximaRevisao:next,titulo:"Revisão preventiva próxima",descricao:"Faltam "+remaining+" km para a revisão prevista. Antecipe o agendamento.",now:now});
  }
}
...
function invalidateAdminSearchCache_(){
  try{CacheService.getScriptCache().remove("SIGVTR_ADMIN_SEARCH_DOCS_V3");}catch(_){}
}
function getAdminSearchDocuments_(){
  const cache=CacheService.getScriptCache(),key="SIGVTR_ADMIN_SEARCH_DOCS_V3";
...
function getAdminDamages_(params){
  const ss=getSpreadsheet_(),vehicles=vehicleIndexAdmin_(ss),withdrawals=readSheetObjects_(ss.getSheetByName(SIGVTR.SHEETS.WITHDRAWALS)),people={},damageRows=readSheetObjects_(ss.getSheetByName(SIGVTR.SHEETS.DAMAGES));
  withdrawals.forEach(function(r){const type=inferChecklistTypeAdmin_(r);people[String(r.ID_RETIRADA||"")]={condutor:joinRankName_(r["Posto/Graduação"],r.Motorista),protocolo:r.Protocolo||"",tipoChecklist:type,origemLabel:type==="FISCAL"?"Fiscal":"Condutor"};});
  let rows=damageRows.map(function(r){const v=vehicles[String(r.ID_VTR)]||{},p=people[String(r.ID_RETIRADA_DETECCAO||"")]||{tipoChecklist:"CONDUTOR",origemLabel:"Condutor"};return {id:r.ID_AVARIA||"",prefixo:v.prefixo||"",item:r.Item||"",descricao:r.Descrição||"",data:formatDateForApi_(r["Data Detecção"]),situacao:r.Situação||"",registradoPor:p.condutor||"",protocolo:p.protocolo||"",tipoChecklist:p.tipoChecklist,origemLabel:p.origemLabel,local:r["Posição/Local"]||r.Item||"",responsavel:r["Responsável Administração"]||"",observacaoAdministracao:r["Observação Administração"]||"",dataUltimaAtualizacao:formatDateForApi_(r["Data da Última Atualização"]),fotosCount:0,historicoCount:0};});
  ...
  if(search)rows=rows.filter(function(r){return [r.id,r.prefixo,r.item,r.descricao,r.registradoPor,r.protocolo,r.tipoChecklist,r.origemLabel,"avaria","avarias"].join(" ").toLowerCase().indexOf(search)>=0;});
...
function inferChecklistTypeAdmin_(row){
  row=row||{};
  let type=String(row["Tipo Checklist"]||"").trim().toUpperCase();
  if(type==="FISCAL"||type==="CONDUTOR")return type;
  const withdrawalType=String(row.Tipo_Retirada||row["Tipo Retirada"]||"").trim().toUpperCase();
  if(withdrawalType.indexOf("FISCAL")>=0)return "FISCAL";
  if(withdrawalType.indexOf("CONDUTOR")>=0)return "CONDUTOR";
  try{const parsed=JSON.parse(String(row.ITENS_JSON||"{}"));type=String(parsed.tipoChecklist||"").trim().toUpperCase();if(type==="FISCAL"||type==="CONDUTOR")return type;}catch(_){}
  return "CONDUTOR";
}
...
const ADMIN_VEHICLE_HEADERS=["ID-VTR","Prefixo","Placa","Chassi","Nº do Motor","RENAVAM","Marca","Modelo","Ano","Combustível","Tipo Combustível","Tipo","Lotação","KM Inicial","KM Atual","Próxima Revisão KM","Antecedência Alerta KM","Status","Cadastro","Observações","Data Cadastro","Última Atualização","Atualizado Por"];
...
function saveAdminVehicle_(p){const sh=ensureAdminVehicleSheet_(),headers=getHeaders_(sh),rows=sh.getDataRange().getValues(),prefix=normalizeVehiclePrefix_(p.prefixo);if(!prefix)throw new Error("Informe o prefixo.");let rowIndex=-1;const idCol=headers.indexOf("ID-VTR"),preCol=headers.indexOf("Prefixo");for(let i=1;i<rows.length;i++){if((p.id&&String(rows[i][idCol])===String(p.id))||normalizeVehiclePrefix_(rows[i][preCol])===prefix){rowIndex=i+1;break;}}const now=new Date(),id=rowIndex>0?String(sh.getRange(rowIndex,idCol+1).getValue()||""):"VTR-"+Utilities.getUuid(),initialKm=Math.max(0,Number(p.kmInicial||0)),currentKm=Math.max(0,Number(p.kmAtual||0)),nextReview=Math.max(0,Number(p.proximaRevisaoKm||0)),reviewLead=Math.max(0,Number(p.antecedenciaAlertaKm===""||p.antecedenciaAlertaKm==null?1000:p.antecedenciaAlertaKm));if(nextReview&&nextReview<currentKm){}if(reviewLead>50000)throw new Error("A antecedência do alerta não pode ultrapassar 50.000 km.");const values={"ID-VTR":id,"Prefixo":prefix,"Placa":String(p.placa||"").toUpperCase(),"Chassi":String(p.chassi||"").toUpperCase(),"Nº do Motor":String(p.motor||"").toUpperCase(),"RENAVAM":String(p.renavam||""),"Marca":p.marca||"","Modelo":p.modelo||"","Ano":p.ano||"","Tipo Combustível":p.tipoCombustivel||"","Tipo":p.tipo||"","Lotação":p.lotacao||"20º BPM","KM Inicial":initialKm,"KM Atual":currentKm,"Próxima Revisão KM":nextReview,"Antecedência Alerta KM":reviewLead,"Status":p.status||"ATIVA","Observações":p.observacoes||"","Última Atualização":now,"Atualizado Por":p.admin||"Administrador"};values.Cadastro=vehicleRegistrationStatus_(values);if(rowIndex<0){values["Data Cadastro"]=now;appendByHeaders_(sh,headers,values);rowIndex=sh.getLastRow();}else{headers.forEach(function(h,i){if(Object.prototype.hasOwnProperty.call(values,h))sh.getRange(rowIndex,i+1).setValue(values[h]);});}sh.getRange(rowIndex,preCol+1).setNumberFormat("@").setValue(prefix);syncAdminReviewFromVehicle_(id,prefix,currentKm,nextReview,reviewLead,now,p.admin||"Administrador");appendLog_(getSpreadsheet_(),{idUsuario:p.admin||"ADMIN",action:"CADASTRO DE VIATURA",referenceId:id,description:"Cadastro atualizado - "+prefix+" | Próxima revisão: "+(nextReview||"não configurada")+" km | Alerta: "+reviewLead+" km antes",device:{},result:"SUCESSO",now:now});invalidateAdminSearchCache_();return {id:id,prefixo:prefix,cadastro:values.Cadastro,proximaRevisaoKm:nextReview,antecedenciaAlertaKm:reviewLead};}
function syncAdminReviewFromVehicle_(idVtr,prefixo,kmAtual,nextReview,lead,now,admin){ensureAdminSheets_();const sh=requireSheet_(getSpreadsheet_(),"REVISOES"),headers=requireHeaders_(sh,ADMIN_REVIEW_HEADERS),rows=sh.getDataRange().getValues(),idCol=headers.indexOf("ID_VTR"),prefixCol=headers.indexOf("Prefixo");let rowIndex=-1;for(let i=1;i<rows.length;i++){if(String(rows[i][idCol]||"")===String(idVtr)||normalizeAdminPrefixSearch_(rows[i][prefixCol])===normalizeAdminPrefixSearch_(prefixo)){rowIndex=i+1;break;}}const values={"ID_REVISAO":rowIndex>0?String(sh.getRange(rowIndex,headers.indexOf("ID_REVISAO")+1).getValue()||""):"REV-"+Utilities.getUuid(),"ID_VTR":idVtr,"Prefixo":prefixo,"Próxima Revisão KM":Number(nextReview)||0,"Antecedência Alerta KM":Math.max(0,Number(lead)||1000),"Status":nextReview?"ATIVA":"INATIVA","Observação":nextReview?"Programação cadastrada no prontuário da viatura.":"Próxima revisão ainda não configurada.","Última Atualização":now};if(rowIndex<0)appendByHeaders_(sh,headers,values);else headers.forEach(function(h,i){if(Object.prototype.hasOwnProperty.call(values,h))sh.getRange(rowIndex,i+1).setValue(values[h]);});if(nextReview)checkPreventiveReviewForVehicle_(idVtr,prefixo,Number(kmAtual)||0,now);}
function importOfficialFleet_(p){let criadas=0,atualizadas=0;OFFICIAL_FLEET_20BPM.forEach(function(v){const existing=getAdminVehicles_().items.find(function(x){return normalizeVehiclePrefix_(x.prefixo)===normalizeVehiclePrefix_(v[0]);});saveAdminVehicle_({id:existing?existing.id:"",prefixo:v[0],placa:v[1],chassi:v[2],motor:v[3],renavam:v[4],status:existing?existing.status:"ATIVA",marca:existing?existing.marca:"",modelo:existing?existing.modelo:"",ano:existing?existing.ano:"",tipoCombustivel:existing?existing.tipoCombustivel:"",tipo:existing?existing.tipo:"",lotacao:"20º BPM",kmInicial:existing?existing.kmInicial:0,kmAtual:existing?existing.kmAtual:0,proximaRevisaoKm:existing?existing.proximaRevisaoKm:0,antecedenciaAlertaKm:existing?existing.antecedenciaAlertaKm:1000,observacoes:existing?existing.observacoes:"Importado da relação oficial da frota do 20º BPM.",admin:p.admin||"Administrador"});existing?atualizadas++:criadas++;});return {criadas:criadas,atualizadas:atualizadas,total:OFFICIAL_FLEET_20BPM.length};}
