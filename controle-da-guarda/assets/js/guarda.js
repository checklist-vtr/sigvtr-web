const GuardPage=(()=>{
  ...
  async function loadContext(){const data=await ApiService.post('guardaContexto',{});state.context=data;state.vehicles=Array.isArray(data.viaturas)?data.viaturas:[];state.otherHistory=Array.isArray(data.vtrsOutrosHistorico)?data.vtrsOutrosHistorico:[];resetIdleTimer();renderApp()}
  function renderApp(){
    hide('loginView');show('appView');
    $('operatorName').textContent=state.context?.operator?.name||state.context?.operator?.login||'Operador';
    $('moduleVersion').textContent=`v${state.context?.moduleVersion||'0.6.5'}`;
