const GuardPage=(()=>{
  ...
  async function loadContext(){const data=await ApiService.post('guardaContexto',{});state.context=data;state.vehicles=Array.isArray(data.viaturas)?data.viaturas:[];state.otherHistory=Array.isArray(data.vtrsOutrosHistorico)?data.vtrsOutrosHistorico:[];state.militaryIndex=Array.isArray(data.militaresBusca)?data.militaresBusca:[];resetIdleTimer();renderApp()}
  function renderApp(){
  ...
  function renderPendingShifts(list){
    ...
    $('pendingShiftsList').innerHTML=list.map(t=>{const r=t.resumo||{};return `<div class="pending-shift-item"><div class="pending-shift-main"><strong>Turno iniciado em ${escapeHtml(formatDateTime(t.inicioEm))}</strong><span>${escapeHtml(r.movimentacoes||0)} movimentações • ${escapeHtml(r.devolvidas||0)} devolvidas • ${escapeHtml(r.emUso||0)} ainda em uso</span><small>O turno anterior permanece preservado e pode ser encerrado depois pelo próprio comandante ou, se necessário, por outro militar.</small></div><button class="btn btn-sm btn-outline-warning" type="button" data-pending-close="${escapeHtml(t.id)}">Encerrar pendente</button></div>`}).join('');
  ...
  function renderVehicleResults(q){
    const norm=vehicleSearchNorm(q),registered=state.vehicles.filter(v=>!norm||[v.prefixo,v.placa,v.modelo].some(x=>vehicleSearchNorm(x).includes(norm))).slice(0,12);
    const historical=norm?state.otherHistory.filter(v=>[v.prefixo,v.placa].some(x=>vehicleSearchNorm(x).includes(norm))).slice(0,6):[];
    let html=registered.map(v=>`<button class="result-item" type="button" data-vehicle-index="${state.vehicles.indexOf(v)}"><span><span class="result-main">${escapeHtml(v.prefixo?`VTR ${v.prefixo}`:(v.placa||'Viatura'))}</span><span class="result-sub">${escapeHtml([v.placa,v.modelo].filter(Boolean).join(' • ')||'Cadastro SIGVTR')}</span></span><i class="bi bi-chevron-right"></i></button>`).join('');
    if(historical.length){html+=`<div class="history-label">Reservas usadas anteriormente</div>`+historical.map((v,i)=>`<button class="result-item history-result" type="button" data-history-index="${state.otherHistory.indexOf(v)}"><span><span class="result-main">VTR ${escapeHtml(v.prefixo||'—')} <small class="history-chip">RESERVA</small></span><span class="result-sub">${escapeHtml(v.placa||'Sem placa')} • Último KM: ${escapeHtml(v.ultimoKm??'—')}</span></span><i class="bi bi-clock-history"></i></button>`).join('')}
    if(!html)html='<div class="text-secondary small py-2">Nenhuma VTR encontrada. Use “Outros” se for uma reserva nova.</div>';
    $('vehicleResults').innerHTML=html;document.querySelectorAll('[data-vehicle-index]').forEach(b=>b.addEventListener('click',()=>selectVehicle(state.vehicles[Number(b.dataset.vehicleIndex)])));document.querySelectorAll('[data-history-index]').forEach(b=>b.addEventListener('click',()=>selectHistoricalOther(state.otherHistory[Number(b.dataset.historyIndex)])));
  }
  function selectHistoricalOther(v){if(!v)return;selectOtherVehicle(v.prefixo,v.placa,v.ultimoKm,v.ultimoUsoEm)}
  function renderOtherHistorySuggestions(){const q=vehicleSearchNorm($('otherPrefix').value||$('otherPlate').value),box=$('otherVehicleHistory');if(!box)return;if(!q){box.innerHTML='';return}const list=state.otherHistory.filter(v=>vehicleSearchNorm(v.prefixo).includes(q)||vehicleSearchNorm(v.placa).includes(q)).slice(0,6);box.innerHTML=list.length?`<div class="history-label">Já utilizada anteriormente</div>`+list.map(v=>`<button class="result-item history-result" type="button" data-other-prefix="${escapeHtml(v.prefixo)}" data-other-plate="${escapeHtml(v.placa)}"><span><span class="result-main">VTR ${escapeHtml(v.prefixo||'—')}</span><span class="result-sub">${escapeHtml(v.placa||'Sem placa')} • Último KM: ${escapeHtml(v.ultimoKm??'—')}</span></span><i class="bi bi-arrow-right-circle"></i></button>`).join(''):'';box.querySelectorAll('[data-other-prefix]').forEach(b=>b.addEventListener('click',()=>selectHistoricalOther(state.otherHistory.find(v=>String(v.prefixo)===b.dataset.otherPrefix&&String(v.placa)===b.dataset.otherPlate))))}
  ...
  function selectOtherVehicle(prefixo,placa,ultimoKm,ultimoUsoEm){state.selectedVehicle={origem:'OUTROS',id:'',prefixo:String(prefixo||'').trim(),placa:String(placa||'').trim().toUpperCase(),modelo:'',ultimoKmConhecido:ultimoKm??null,ultimoUsoEm:ultimoUsoEm||''};$('otherPrefix').value=String(prefixo||'');$('otherPlate').value=String(placa||'').toUpperCase();renderSelectedVehicle();enableMilitary();resetMilitary()}
  function renderSelectedVehicle(){const v=state.selectedVehicle;if(!v){hide('selectedVehicle');show('vehiclePicker');return}const extra=v.origem==='OUTROS'&&v.ultimoKmConhecido!==null&&v.ultimoKmConhecido!==undefined?` • Último KM ${v.ultimoKmConhecido}`:'';$('selectedVehicle').innerHTML=`<div><strong>${escapeHtml(v.prefixo?`VTR ${v.prefixo}`:'VTR')}</strong><small>${escapeHtml([v.placa,v.modelo,v.origem==='OUTROS'?'Reserva / Outros':''].filter(Boolean).join(' • '))}${escapeHtml(extra)}</small></div><button id="changeVehicle" class="btn btn-sm btn-outline-primary" type="button">Alterar</button>`;show('selectedVehicle');hide('vehiclePicker');hide('otherVehicleForm');$('changeVehicle').addEventListener('click',()=>{state.selectedVehicle=null;resetMilitary();renderSelectedVehicle();$('militaryCard').classList.add('is-disabled');$('militarySearch').disabled=true;hide('reviewCard')});updateReview()}
  ...
  function militarySearchNorm(v){return String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim()}
  function localMilitarySearch(q,limit=20){const raw=String(q||'').trim(),digits=raw.replace(/\D/g,''),text=militarySearchNorm(raw);if(raw.length<2)return [];const out=[];for(const m of state.militaryIndex){const rg=String(m.rg||'').replace(/\D/g,''),digitMatch=digits&&rg.includes(digits),textMatch=text&&(militarySearchNorm(m.nomeCompleto).includes(text)||militarySearchNorm(m.nomeGuerra).includes(text));if(digitMatch||textMatch){out.push(m);if(out.length>=limit)break}}return out}
  function upsertMilitaryIndex(m){if(!m||!m.id)return;const i=state.militaryIndex.findIndex(x=>x.id===m.id);if(i>=0)state.militaryIndex[i]=m;else state.militaryIndex.push(m)}
  async function searchMilitary(){const q=$('militarySearch').value.trim();$('militaryResults').innerHTML='';hide('newMilitaryButton');if(q.length<2)return;let list=localMilitarySearch(q,20);if(!list.length&&/^\d{6,11}$/.test(q.replace(/\D/g,''))){try{const data=await ApiService.post('guardaPesquisarMilitar',{query:q,limit:20});list=data.militares||[]}catch(_){}}renderMilitaryResults(list);if(!list.length)show('newMilitaryButton')}
  ...
  async function searchCommander(){
    ...
    let list=localMilitarySearch(q,12);show('commanderSearchResults');
    if(!list.length&&/^\d{6,11}$/.test(q.replace(/\D/g,''))){try{const data=await ApiService.post('guardaPesquisarMilitar',{query:q,limit:12});list=Array.isArray(data.militares)?data.militares:[]}catch(_){}}
  ...
  async function openCloseShift(turnoId='',substitute=false){
    clearAlert();clearAlert('closeShiftAlert');clearCommanderFields();state.closeMode=substitute?'PENDENTE':'NORMAL';state.closeTurnId=turnoId||'';
    $('closeShiftTitle').textContent=substitute?'Encerrar turno pendente':'Fechar turno da Guarda';
    $('closeShiftContext').textContent=substitute?'Informe quem está realizando o encerramento deste turno pendente.':'Confirme os dados do Comandante da Guarda deste serviço.';
    if(substitute){show('pendingCloserGroup');document.querySelectorAll('input[name="pendingCloserType"]').forEach(r=>{r.checked=false});hide('substituteReasonGroup');$('substituteReason').required=false;$('substituteReason').value='';$('confirmCloseShiftButton').textContent='Confirmar e encerrar turno pendente'}else{hide('pendingCloserGroup');hide('substituteReasonGroup');$('substituteReason').required=false;$('substituteReason').value='';$('confirmCloseShiftButton').textContent='Confirmar dados e fechar turno'}
    ...
    try{let r=null;if(substitute){const pend=(state.context?.turnosPendentes||[]).find(t=>String(t.id)===String(turnoId));r=pend?.resumo||null}else{const list=Array.isArray(state.context?.movimentacoes)?state.context.movimentacoes:[];const currentId=state.context?.turno?.id||'';if(currentId){const relevant=list.filter(m=>String(m.idTurnoRetirada||m.idTurno||'')===String(currentId)||String(m.idTurnoDevolucao||'')===String(currentId)||['AGUARDANDO_CONFIRMACAO_RETIRADA','EM_USO','AGUARDANDO_CONFIRMACAO_DEVOLUCAO'].includes(String(m.status||'')));const devolvidas=relevant.filter(m=>String(m.status||'')==='ENCERRADA'&&String(m.idTurnoDevolucao||'')===String(currentId)).length,emUso=relevant.filter(m=>['EM_USO','AGUARDANDO_CONFIRMACAO_DEVOLUCAO'].includes(String(m.status||''))).length,aguardandoConfirmacao=relevant.filter(m=>String(m.status||'').startsWith('AGUARDANDO_CONFIRMACAO')).length,devolucoesTurnoAnterior=relevant.filter(m=>String(m.status||'')==='ENCERRADA'&&String(m.idTurnoRetirada||m.idTurno||'')!==String(currentId)&&String(m.idTurnoDevolucao||'')===String(currentId)).length;r={movimentacoes:relevant.length,devolvidas,emUso,aguardandoConfirmacao,devolucoesTurnoAnterior}}}if(!r){const data=await ApiService.post('guardaPreviaFechamento',substitute?{turnoId:turnoId}:{});r=data.resumo||{}};
  ...
  async function submitCloseShift(event){event.preventDefault();clearAlert('closeShiftAlert');const pending=state.closeMode==='PENDENTE',btn=$('confirmCloseShiftButton');let encerramentoPorOutro=false;
    ...
    try{
      const data=await ApiService.post(pending?'guardaEncerrarTurnoPendente':'guardaFecharTurno',payload,{timeout:30000,retries:0});
      ...
      try{
        ...
        hide('closeShiftLoading');show('closeShiftSuccess');$('closeShiftSuccessText').textContent=`${pending?'Turno pendente':'Turno'} encerrado por ${[resp.postoGraduacao,resp.nomeGuerra].filter(Boolean).join(' ')}. O PDF está pronto.`;$('downloadShiftPdfButton').disabled=false;setTimeout(()=>loadContext().catch(()=>{}),250);
      }catch(pdfError){
    ...
    }catch(error){hide('closeShiftLoading');show('closeShiftContent');alertBox(error.message||'Não foi possível encerrar o turno.','danger','closeShiftAlert')}finally{btn.disabled=false;btn.textContent=pending?'Confirmar e encerrar turno pendente':'Confirmar dados e fechar turno'}
  }
  ...
  function bind(){
    ...
    $('vehicleSearch').addEventListener('input',e=>renderVehicleResults(e.target.value));$('otherVehicleButton').addEventListener('click',()=>{hide('vehiclePicker');show('otherVehicleForm');renderOtherHistorySuggestions();$('otherPrefix').focus()});$('cancelOtherVehicle').addEventListener('click',()=>{hide('otherVehicleForm');show('vehiclePicker')});$('otherVehicleForm').addEventListener('submit',e=>{e.preventDefault();const p=$('otherPrefix').value.trim(),pl=$('otherPlate').value.trim();if(!p||!pl)return;selectOtherVehicle(p,pl)});$('otherPrefix').addEventListener('input',renderOtherHistorySuggestions);$('otherPlate').addEventListener('input',renderOtherHistorySuggestions);
    $('militarySearch').addEventListener('input',()=>{clearTimeout(state.searchTimer);state.searchTimer=setTimeout(searchMilitary,120)});$('newMilitaryButton').addEventListener('click',()=>openMilitaryForm({rg:/^\d+$/.test($('militarySearch').value.trim())?$('militarySearch').value.trim():'',nomeCompleto:/\D/.test($('militarySearch').value)?$('militarySearch').value.trim():''}));$('militaryForm').addEventListener('submit',saveMilitary);$('cancelMilitaryForm').addEventListener('click',()=>{hide('militaryForm');show('militaryPicker')});
