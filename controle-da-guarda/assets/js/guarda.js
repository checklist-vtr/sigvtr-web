const GuardPage=(()=>{
  ...
  function renderApp(){
    ...
    $('moduleVersion').textContent=`v${state.context?.moduleVersion||'0.6.0'}`;
  ...
  function renderQr(data,operation){
    ...
    hide('qrSuccess');show('qrWaiting');show('qrOverlay');startPolling(movement.id,state.currentOperation);refreshMovements(false)
  }
  ...
  async function submitCloseShift(event){event.preventDefault();clearAlert('closeShiftAlert');const substitute=state.closeMode==='SUBSTITUTO',btn=$('confirmCloseShiftButton');btn.disabled=true;btn.innerHTML='<span class="spinner-border spinner-border-sm me-2"></span>Fechando turno e gerando PDF...';
    ...
    try{const data=await ApiService.post(substitute?'guardaEncerrarTurnoPendente':'guardaFecharTurno',payload,{timeout:45000,retries:0});const resp=data.responsavel||data.comandante||{};state.lastPdf=data.pdf||null;hide('closeShiftContent');hide('closeShiftLoading');show('closeShiftSuccess');$('closeShiftSuccessText').textContent=data.pdf?`${substitute?'Turno pendente':'Turno'} encerrado por ${[resp.postoGraduacao,resp.nomeGuerra].filter(Boolean).join(' ')}. O PDF está pronto.`:`Turno encerrado, mas o PDF não pôde ser gerado agora: ${data.pdfErro||'erro não informado'}.`;$('downloadShiftPdfButton').disabled=!data.pdf;await loadContext()}catch(error){alertBox(error.message||'Não foi possível encerrar o turno.','danger','closeShiftAlert')}finally{btn.disabled=false;btn.textContent=substitute?'Confirmar e encerrar turno pendente':'Confirmar dados e fechar turno'}
  }
  ...
  function bind(){
    ...
    $('closeShiftButton').addEventListener('click',()=>openCloseShift('',false));$('closeShiftCloseButton').addEventListener('click',closeCloseShift);$('commanderSearch').addEventListener('input',()=>{clearTimeout(state.commanderSearchTimer);state.commanderSearchTimer=setTimeout(searchCommander,280)});$('closeShiftForm').addEventListener('submit',submitCloseShift);$('downloadShiftPdfButton').addEventListener('click',()=>{if(!downloadPdf(state.lastPdf))alertBox('PDF indisponível para download.','warning')});$('finishCloseShiftButton').addEventListener('click',closeCloseShift);$('passwordForm').addEventListener('submit',changeGuardPassword);$('logoutButton').addEventListener('click',logout)
  }
